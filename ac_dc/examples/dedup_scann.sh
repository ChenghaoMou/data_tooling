...
for lang in "${LANGUAGES[@]}"; do
    ...
    echo "Gather hashes"
    $PYTHON $SCRIPT gather-hashes "cache/sharded_deduplicated_${lang}/simhash_index.hdf5" $(seq -s " " -f "cache/sharded_deduplicated_${lang}/hashes_%05g" 0 "$((SHARDS - 1))") --split "train"
    
    echo "Finding duplicates"
    for i in $(seq -f "%05g" 0 "$((SHARDS - 1))"); do
        echo "Querying shard ${i}"
        $PYTHON -W ignore $SCRIPT find-scann-duplicates "cache/sharded_deduplicated_${lang}/hashes_${i}" "cache/sharded_deduplicated_${lang}/simhash_index.hdf5" --split "train" --k 100 --threshold $THRESHOLD
    done
